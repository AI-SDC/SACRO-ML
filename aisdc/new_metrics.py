...
def new_get_metrics(  # pylint: disable=too-many-locals
    y_test: np.ndarray,
    ...
    if len(shape) != 2:
        raise ValueError("Insert helpful error message here")
    else:
        "PASS 1"
    ...
    try:
        ...
        print("I need a floating point number to continue.")
...
if __name__ == "__main__":
    ...
    print(y_pred.shape)
    print(y_test.shape)
    print("This should fail about here...")

    print(new_get_metrics(y_test, y_pred))
    ...
    x_train, x_test, y_train, y_test = train_test_split(np.array(y_pred), np.array(y_label), test_size=0.2, random_state=42)

    attack_rf.fit(x_train,y_train)
    ...
    if permute_rows:
        ...
        x_test = x_test[order, :]
        y_test = y_test[order]
    y_pred = attack_rf.predict_proba(x_test)

    before = get_metrics(attack_rf, x_test, y_test, permute_rows=False)
    after = new_get_metrics(y_test, y_pred)
