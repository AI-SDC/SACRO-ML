...
def get_metrics(clf,
    ...
    # TPR at various FPR
    fpr_vals = [0.5, 0.2, 0.1, 0.01, 0.001, 0.0001, 0.00001] # taken from https://arxiv.org/pdf/2112.03570.pdf
    for fpr in fpr_vals:
...
def main():
    '''Quick test method so that the code can be run to test funcionality'''
    ...
    trainX, testX, trainy, testy = train_test_split(X, permy, test_size=0.3)
    rf.fit(trainX, trainy)
    m = get_metrics(rf, testX, testy)
    print("PERMUTED")
    for k, v in m.items():
...
if __name__ == '__main__':
    # Code to just run this to check the results look meaningful
    main()
