"""Quantile Membership Inference Attack for tabular classification."""
...
class QMIAAttack(Attack):
    ...
    def attackable(cls, target: Target) -> bool:  # pragma: no cover
        ...
        labels = np.hstack((target.y_train, target.y_test))
        if np.unique(labels).size != 2:
            logger.info("WARNING: QMIA v1 supports binary classification only.")
            return False
    ...
    def _attack(self, target: Target) -> dict:
        ...
        train_scores = utils.qmia_binary_score(proba_train, target.y_train)
        test_scores = utils.qmia_binary_score(proba_test, target.y_test)

        regressor = self._fit_regressor(target.X_test, test_scores)

        combined_x = np.vstack((target.X_train, target.X_test))
        combined_scores = np.hstack((train_scores, test_scores))
        thresholds = self._predict_thresholds(regressor, combined_x)
    ...
    def _construct_metadata(self) -> None:
        ...
        self.metadata["global_metrics"]["qmia_score"] = "binary_true_label_logit"
