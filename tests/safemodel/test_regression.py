...
def regression_target():
    ...
    return Target(model=model, X_train=x[:140], y_train=y[:140], X_test=x[140:], y_test=y[140:])
...
def test_regression_target_roundtrip(regression_target, tmp_path):
    ...
    np.testing.assert_allclose(restored.model.get_losses(restored.X_test, restored.y_test), expected)
...
def test_worstcase_regression(regression_target, tmp_path, include_error, monkeypatch):
    ...
    attack = WorstCaseAttack(output_dir=str(tmp_path), write_report=True, n_reps=2, n_dummy_reps=0,
    ...
    if include_error:
        ...
        np.testing.assert_allclose(captured[0][:140, 1], regression_target.model.get_losses(regression_target.X_train, regression_target.y_train))
...
def test_lira_regression(regression_target, tmp_path, mode):
    ...
    attack = LIRAAttack(output_dir=str(tmp_path), write_report=False, n_shadow_models=12,
                       mode=mode, report_individual=True)
...
def test_qmia_regression(regression_target, tmp_path):
    ...
    attack = QMIAAttack(output_dir=str(tmp_path), write_report=True, max_iter=40,
                        alpha=0.1, report_individual=True)
    output = attack.attack(regression_target)
    ...
    np.testing.assert_allclose(individual["margin"],
                               np.asarray(individual["score"]) - individual["threshold"])
    assert (tmp_path / "report.json").exists()


@pytest.mark.parametrize("estimator", [DecisionTreeRegressor(random_state=0),
...
def test_structural_regression(regression_target, tmp_path, estimator):
    ...
    attack = StructuralAttack(output_dir=str(tmp_path), write_report=True, report_individual=True)
    output = attack.attack(target)
    values = instance(output)
    ...
    assert values["generalisation_gap"] == pytest.approx(values["test_mse"] - values["train_mse"])
    assert (tmp_path / "report.pdf").exists()
...
def test_regression_mlp_parameter_count():
    ...
    model = MLPRegressor(hidden_layer_sizes=(2,), solver="lbfgs", random_state=0).fit([[0], [1], [2], [3]], [0, 1, 2, 3])
...
def test_attribute_regression_identifies_categorical_values(tmp_path):
    ...
    x = np.array([[a, b] for a in (0., 1., 2.) for b in (0., 1., 2.)])
    y = 2 * x[:, 0] + 0.25
    target = Target(model=LinearRegression().fit(x, y), X_train=x, X_test=x,
                    y_train=y, y_test=y, X_train_orig=x, X_test_orig=x,
                    y_train_orig=y, y_test_orig=y)
    target.add_feature("signal", [0], "int")
    target.add_feature("irrelevant", [1], "int")
    output = AttributeAttack(output_dir=str(tmp_path), write_report=True, n_cpu=1).attack(target)
...
def test_attribute_regression_numeric_bounds(value):
    ...
    x = np.array([[a, b] for a in np.linspace(-3, 3, 11) for b in (0., 1.)])
...
def test_meta_regression_repeated_structural(regression_target, tmp_path):
    ...
    attack = MetaAttack(attacks=[("structural", {}, 2)], output_dir=str(tmp_path), write_report=False)
    output = attack.attack(regression_target)
    assert output["metadata"]["target_task"] == "regression"
    ...
    assert members["struct_vuln"].tolist() == (members["struct_k"] < attack.k_threshold).tolist()
...
def test_instance_based_regression(regression_target, tmp_path):
    ...
    target = regression_target
    target.model = SklearnModel(KNeighborsRegressor().fit(target.X_train, target.y_train))
    output = InstanceBasedAttack(output_dir=str(tmp_path), write_report=False).attack(target)
    assert output
    assert output["metadata"]["target_task"] == "regression"


def test_qmia_nonfinite_regression_fails_explicitly(regression_target, tmp_path, monkeypatch):
    """A model returning NaN cannot produce a successful risk report."""
    monkeypatch.setattr(regression_target.model, "get_losses", lambda x, y: np.full(len(x), np.nan))
    output = QMIAAttack(output_dir=str(tmp_path), write_report=False).attack(regression_target)
...
def test_regression_pipeline_losses_and_clone(regression_target):
    ...
    target = regression_target
    model = SklearnModel(make_pipeline(StandardScaler(), LinearRegression()).fit(target.X_train, target.y_train))
    clone = model.clone()
    clone.fit(target.X_train, target.y_train)
    assert clone.is_regression
    np.testing.assert_allclose(clone.predict(target.X_test), model.predict(target.X_test))
