...
def _print_summary(rows: list[dict[str, Any]]) -> None:
    ...
    print("\nBenchmark summary:")
    header = f"{'scenario':<16} {'attack':<14} {'secs':>8} {'AUC':>8} {'Adv':>8} {'TPR':>8} {'FPR':>8}"
    print(header)
    print("-" * len(header))
    for row in rows:
        print(
            ...
            f"{row.get('attack', ''):<14} "
            f"{row.get('seconds', float('nan')):>8.4f} "
            f"{row.get('AUC', float('nan')):>8.4f} "
            f"{row.get('Advantage', float('nan')):>8.4f} "
            f"{row.get('TPR', float('nan')):>8.4f} "
            f"{row.get('FPR', float('nan')):>8.4f}"
        )
