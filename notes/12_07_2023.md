...
## Agenda (12 July 2023)
1. Version 1.0.6 (Discussion on the assigned tasks - given completion date of 14 July 2023)
...
2. Discussion on 'A simple attack on probabilities' (https://github.com/AI-SDC/AI-SDC/issues/202)
3. User Stories (Discussion)
   - We agreed to focus on user stories from 1 to 3 in this sprint
   - Folder Structure - summary.txt, raw_output.json, target.json, and the attack model
   - code cleaning up - Should we use make_report() with no argument to make it clear as it was in the earlier versions
     - from the command line we can't pass through the name of the json output file in the current version rather a hard coded name is used to generate json output
4. Planning for Sprint 3
   - SM: replicating 'Difficulty of MI' paper using our metrics - Update
5. AOB
6. Next week's chair
   - JL
