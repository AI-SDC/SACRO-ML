...
## 2 Actions from last week
  ...
  - [ x] **ACTION CC**: Findout expected date for access to non-TRE AWS we can all have acces to
...
**DECISION**: In most cases, especially for complex models, we don't need repeat runs creating multiple target models with the same hyper-paramters.— instead of using seed that can change for reproducing test/train split use different parameter separetely
...
- **CC** Give access to non-TRE AWS we can all have acces to – need access SM, AC, RP, YJ, JS, JL, MA
