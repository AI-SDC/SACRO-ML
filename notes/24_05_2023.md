...
0. Forward planning
    * Quick gentle reminder to all as we plan things to keep PRs narrowly scoped to a single fix/feature and linked to an issue that has detail.
...
2. v.1.0.6 (July 3, 2023?)
    * Plan: Getting to `TARGET.json` :heavy_plus_sign: `ATTACK_CONFIG.json` :arrow_right: `ATTACKS.json` :arrow_right: `SUMMARY.txt`
        * Improve `ATTACK_CONFIG.json`
            * Replace the attack args classes with a dictionary within each attack class and move config file handling into the `Attack` class [Issue [#156](https://github.com/AI-SDC/AI-SDC/issues/156)] [Assigned: ?]
        ...
        * Create `ATTACKS.json` 
            ...
            * Ensure that `ATTACKS.json` can be produced on the command line from `TARGET.json` and `ATTACK_CONFIG.json` - especially where `TARGET.json` includes a path to predicted probabilities instead of raw data this helps support R etc. [Assigned: ?]
        * Report generation should produce `summary.txt` by taking as input the `TARGET.json` and `ATTACK.json` files (*depends on completion of above*) [Issue [#152](https://github.com/AI-SDC/AI-SDC/issues/152)] [Assigned: ?]
        ...
        * Add `SafeXGBoostClassifier` (*depends on completion of above*) [Issue [#157](https://github.com/AI-SDC/AI-SDC/issues/157)] [Assigned: ?]
        ...
        * Add generalisation error reporting to safemodels (and include in `TARGET.json`) [Issue [#150](https://github.com/AI-SDC/AI-SDC/issues/150)] [Assigned: ?] [Assigned: ?]
