...
2. v.1.0.6 (July 3, 2023?)
    * Plan: Getting to `TARGET.json` :heavy_plus_sign: `ATTACK_CONFIG.json` :arrow_right: `ATTACKS.json` :arrow_right: `SUMMARY.txt`
        ...
        * Create `ATTACKS.json` - redirect attack output to a single file instead of creating individual files for each attack [Assigned: ?]
        * Report generation should produce `summary.txt` by taking as input the `TARGET.json` and `ATTACKS.json` files (*depends on completion of above*) [Issue [#152](https://github.com/AI-SDC/AI-SDC/issues/152)] [Assigned: ?]
