...
from aisdc.attacks.attack_report_formatter import GenerateTextReport, GenerateJSONModule
from aisdc.attacks.worst_case_attack import WorstCaseAttack
from aisdc.attacks.likelihood_attack import LIRAAttack

def generate_report(directory,target_model,x_train,y_train,x_test,y_test,attack_output_name,target_filename,outfile):
    """
    ...
    # Run the attack
    wca = WorstCaseAttack(n_dummy_reps=10, report_name=directory+'/disclosive_model_raw_output')
    wca.attack(target)
    ...
    t.process_attack_target_json(directory+attack_output_name,target_filename=directory+target_filename)
...
def main():
    ...
    parser.add_argument(
        ...
        default='trainX.txt',
        help=(
            "Filename for the saved training data. Default = %(default)s."
        ),
    )
    ...
    parser.add_argument(
        ...
        default='trainy.txt',
        help=(
            "Filename for the saved training labels. Default = %(default)s."
        ),
    )
    ...
    parser.add_argument(
        ...
        default='testX.txt',
        help=(
            "Filename for the saved testiing data. Default = %(default)s."
        ),
    )
    ...
    parser.add_argument(
        ...
        default='testy.txt',
        help=(
            "Filename for the saved testing labels. Default = %(default)s."
        ),
    )
